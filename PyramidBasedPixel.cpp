...
int PyramidBasedPixelAlignment::RunPyramidBasedPixelAlignment(MotionModel model = MotionModel::AFFINE)
{
	...
	for (int i = (numLevels - 1); i >= 0; i--)
	{
		try
		{
			findTransformECC(img1Pyr[i], img2Pyr[i], warpMat, model, exitCriteria[i]);
		}
		catch (exception e)
		{
		...
		if (i != 0)
		{
			warpMat = warpMat.mul(scaleMat);
		}
	}
	
	if (model == MotionModel::HOMOGRAPHY)
	{
		warpPerspective(origImg2, this->alignedImg, warpMat, origImg2.size(), INTER_LINEAR + WARP_INVERSE_MAP);
	}
	else
	{
		warpAffine(origImg2, this->alignedImg, warpMat, origImg2.size(), INTER_LINEAR + WARP_INVERSE_MAP);
	}
	return retVal;
}
