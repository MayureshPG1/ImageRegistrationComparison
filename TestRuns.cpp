/* Contains various methods to perform performance and quality test on images*/
...
namespace TestRuns
{
	...
	//bufferImg is the image in which output of Max operation is saved
	// it is a running Max buffer
	// For first time use pass bufferImg with all zeros
	int GetMaxImage(Mat& bufferImg, Mat ipImg)
	{
		//convert to gray
		Mat grayBufferImg, grayIpImg;
		cvtColor(bufferImg, grayBufferImg, CV_BGR2GRAY);
		cvtColor(ipImg, grayIpImg, CV_BGR2GRAY);

		// find max gray value btw the two and put bigger rgb value back into rgb buffer
		for (int i = 0; i < grayBufferImg.rows; i++)
		{
			for (int j = 0; j < grayBufferImg.cols; j++)
			{
				if (grayBufferImg.at<uchar>(i, j) <= grayIpImg.at<uchar>(i, j))
				{
					bufferImg.at<Vec3b>(i, j) = ipImg.at<Vec3b>(i, j);
				}
			}
		}
	...
	int RunPSNRTest()
	{
		...
		for (int i = 0; i < listDir.folders.size(); i++)
		{
			...
			FeatureBasedAlignment fxReg(img1, img2, ScalingFactor::HALF);
			...
			int retVal = fxReg.RunFeatureBasedAlignment(FeatureType::fORB, MatchType::RATIO_SYMMETRY);
	...
	int RunFeatureMatchScaleTest()
	{
		...
		for (int i = 0; i < listDir.folders.size(); i++)
		{
			...
			int retVal;
			vector<ScalingFactor> scales{ ScalingFactor::FULL,ScalingFactor::HALF,ScalingFactor::QUARTER };
			...
			for (int i = 0; i < scales.size(); i++)
			{
				...
				FeatureBasedAlignment fxReg(img1, img2, scales[i]);
				...
				e1 = cv::getTickCount();
				// pass returned warpmat to next iteration if required
				retVal = fxReg.RunFeatureBasedAlignment(FeatureType::fORB, MatchType::BASIC);
				e2 = cv::getTickCount();
				img2Aligned = fxReg.GetAlignedImage();
				RemoveBlackBorder(img2Aligned, img1);
				if (retVal == -1)
				...
				e1 = cv::getTickCount();
				// pass returned warpmat to next iteration if required
				retVal = fxReg.RunFeatureBasedAlignment(FeatureType::fORB, MatchType::RATIO_SYMMETRY);
	...
	int RunFeatureMatchTest()
	{
		...
		for (int i = 0; i < listDir.folders.size(); i++)
		{
			...
			double imgPsnr = GetPSNR(img1, img2);
			int retVal;
			FeatureBasedAlignment fxReg(img1, img2);
			...
			e1 = cv::getTickCount();
			// pass returned warpmat to next iteration if required
			retVal = fxReg.RunFeatureBasedAlignment(FeatureType::fORB, MatchType::FILTER_SYMMETRY);
	...
	int MultiImage_FinCorpus_Scale()
	{
		...
		for (int i = 0; i < listDir.folders.size(); i++)
		{
			...
			for (int j = 1; j < inputImgNames.size(); j++)
			{
				...
				e1 = cv::getTickCount();
				OpticalFlowAlignment optFlowAlignment(img1, img2, ScalingFactor::HALF);
				...
				OpticalFlowAlignment optFlowAlignment1(img1, img2, ScalingFactor::QUARTER);
				e1 = cv::getTickCount();
	...
	//combine output of 1..N images into single image as average
	int MultiImage_FinCorpus_MultiBlend()
	{
		...
		opFile.open(folderpath + "MultiImage_Multiblend.csv");

		opFile << " ,ORB-ratio-Symmetry,optflow-0.25fwbw,ORB-ratio-symmetry-time,optflow-0.25fwbw-time,\n";
		for (int i = 0; i < listDir.folders.size(); i++)
		{
			string newFolderPath = folderpath + listDir.folders[i] + "\\";
			...
			string imgName = listDir.folders[i] + "-" + inputImgNames.front() + "-to-" + inputImgNames.back();
			opFile << imgName + ",";
			ss.str("");
			...
			Mat avgImgOptFlow = Mat::zeros(img1.size(), img1.type());
			Mat accumulatorOptFlow = Mat::zeros(img1.size(), CV_32FC3);
			double psnrOptFlowFWBW025, toptflowFWBW025;

			Mat avgImgORB = Mat::zeros(img1.size(), img1.type());
			Mat accumulatorORB = Mat::zeros(img1.size(), CV_32FC3);
			double psnrorbratio, torbratio;

			img1.convertTo(accumulatorOptFlow, CV_32FC3);
			img1.convertTo(accumulatorORB, CV_32FC3);
			torbratio = toptflowFWBW025 = 0.0;
			...
			for (int j = 1; j < inputImgNames.size(); j++)
			{
				...
				e1 = cv::getTickCount();
				OpticalFlowAlignment optFlowAlignment1(img1, img2, ScalingFactor::QUARTER);
				int retVal = optFlowAlignment1.RunOpticalFlowAlignment_OPT(true, false, false);
				...
				cv::accumulateWeighted(img2Aligned, accumulatorOptFlow, 0.5);
				...
				cv::accumulateWeighted(img2Aligned, accumulatorORB, 0.5);
				e2 = cv::getTickCount();
				
				torbratio += (e2 - e1) / cv::getTickFrequency();
			}

			convertScaleAbs(accumulatorOptFlow, avgImgOptFlow);
			psnrorbratio = psnrOptFlowFWBW025 = 0;
			line1 = imgName + "-OptFlow-FWBW_0_25_Scale" + "-Multiblend.jpg";
			ss << "PSNR:" << psnrOptFlowFWBW025 << "Avg Exe TIme:" << (toptflowFWBW025 / (inputImgNames.size() -1));
			line2 = ss.str();
			ss.str("");
			//write avg image as output
			OverlayText(avgImgOptFlow, line1, line2);
			imwrite(newFolderPath + line1, avgImgOptFlow);


			convertScaleAbs(accumulatorORB, avgImgORB);
			line1 = imgName + "-0.5" + "-ORB" + "-mRatioSymmetry" + "-Multiblend.jpg";
			ss << "PSNR:" << psnrorbratio << " Exe TIme:" << (torbratio / (inputImgNames.size() - 1));
			line2 = ss.str();
			ss.str("");
			//write avg image as output
			OverlayText(avgImgORB, line1, line2);
			imwrite(newFolderPath + line1, avgImgORB);
	...
	// combine output of multi image 1..N by using max filter
	int MultiImage_FinCorpus_MultiMaxBlend()
	{
		Mat img1, img2;

		//get all the folders
		string folderpath = "C:\\Projects\\Noctacam\\Registration\\bigdata1\\";
		List listDir = lsfiles(folderpath);
		...
		opFile.open(folderpath + "MultiImage_MultiMaxBlend.csv");

		opFile << " ,optflow-0.25fwbw,ORB-ratio-Symmetry,optflow-0.25fwbw-time,ORB-ratio-symmetry-time,\n";
		for (int i = 0; i < listDir.folders.size(); i++)
		{
			string newFolderPath = folderpath + listDir.folders[i] + "\\";
			...
			for (int l = 0; l < listDir1.files.size(); l++)
			{
				//if (listDir1.files[l].find("jpg") != string::npos)
				//if (listDir1.files[l].length() == 11)//use only 'input *.jpg' not the saved output files
				//Now all kinds of names are allowed. Names are sorted and first image is taken as reference image wrt all rest of images are aligned.
				{
					inputImgNames.push_back(listDir1.files[l]);
				}
			}

			//output images filtering logic. Must remove these in all test cases otherwise code will even include output images as input
			vector<int> removeIndices;
			for (int l = 0; l < inputImgNames.size(); l++)
			{
				if (inputImgNames[l].find("-MultiMaxBlend") != string::npos)
				{
					removeIndices.push_back(l);
				}
			}

			for (int k = 0; k < removeIndices.size(); k++)
			{
				inputImgNames.erase(inputImgNames.begin() + removeIndices[k]);
			}
			...
			//initialization
			torbratio = toptflowFWBW025 = 0.0;
			TestRuns::GetMaxImage(avgImgOptFlow, img1);
			TestRuns::GetMaxImage(avgImgORB, img1);
			...
			for (int j = 1; j < inputImgNames.size(); j++)
			{
				...
				//max image logic
				TestRuns::GetMaxImage(avgImgOptFlow, img2Aligned);
				...
				//max image logic
				TestRuns::GetMaxImage(avgImgORB, img2Aligned);
			...
			psnrorbratio = psnrOptFlowFWBW025 = 0;
			line1 = imgName + "-OptFlow-FWBW_0_25_Scale" + "-MultiMaxBlend.jpg";
			ss << "PSNR:" << psnrOptFlowFWBW025 << "Avg Exe TIme:" << (toptflowFWBW025 / (inputImgNames.size() - 1));
			line2 = ss.str();
			ss.str("");
			//write avg image as output
			...
			line1 = imgName + "-0.5" + "-ORB" + "-mRatioSymmetry" + "-MultiMaxBlend.jpg";
			ss << "PSNR:" << psnrorbratio << " Exe TIme:" << (torbratio / (inputImgNames.size() - 1));
			line2 = ss.str();
			ss.str("");
			//write avg image as output
